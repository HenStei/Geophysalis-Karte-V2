...
import { X, Upload, MapPin, Check, Info } from 'lucide-react';
...
export default function MapView() {
  ...
  // States für den neuen Pin-Workflow
  ...
  // 1. Klick auf "Sticker setzen" - Pin in die Mitte der Karte legen
  ...
  // Drag-Logik für den Entwurfs-Pin
  ...
  // 2. Klick auf "Position bestätigen" öffnet das Upload-Modal
  const handleConfirmPosition = () => {
    setIsModalOpen(true);
    setExifNotice("");
  };

  // 3. Bildauswahl und EXIF-Scan
  const handleImageSelection = async (e) => {
    ...
    try {
      ...
      if (gps && gps.latitude && gps.longitude) {
         ...
         setExifNotice("Standort aus Foto-Daten (EXIF) übernommen!");
         // Karte zum neuen Ort bewegen
  ...
  // Lat/Lng Manuelle Änderung (falls der Nutzer tippen will)
  ...
  return (
    <div className="relative w-full h-full">
      ...
      {/* Button zum Starten ODER Panel zum Bestätigen */}
      {!draftPin ? (
        <button 
          ...
          <div className="bg-white rounded-3xl w-full max-w-md p-6 relative shadow-2xl my-auto">
            ...
            <div className="mb-4 bg-gray-50 p-3 rounded-xl border border-gray-100">
              ...
              <div className="flex gap-2">
                <input type="number" value={draftPin[0].toFixed(5)} onChange={handleLatChange} className="w-1/2 text-xs border border-gray-300 p-2 rounded-lg bg-white" placeholder="Breitengrad" />
                <input type="number" value={draftPin[1].toFixed(5)} onChange={handleLngChange} className="w-1/2 text-xs border border-gray-300 p-2 rounded-lg bg-white" placeholder="Längengrad" />
              </div>
            </div>
